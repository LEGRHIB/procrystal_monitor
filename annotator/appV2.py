...
def _incremental_export_frame(exp_name: str, frame_idx: int):
    ...
    try:
        ...
        if crop_region and frame_idx < len(frames):
            ...
            # Support droplet subfolder if stored in settings
            _droplet = settings.get("droplet")
            img = cv2.imread(str(resolve_raw_path(exp_name, frame_file, _droplet)))
            if img is not None:
...
def scan_experiment(exp_path: Path, droplet: str = None) -> list[dict]:
    ...
    If ``droplet`` is given (e.g. "droplet_001"), images are read from
    raw_images/<droplet>/ instead of raw_images/.  This supports the
    gantry multi-droplet folder layout while staying backwards-compatible
    with single-droplet experiments.
    """
...
def list_droplets(exp_name: str) -> list[dict]:
    """Return droplet subfolders inside raw_images/ (for gantry multi-droplet experiments).

    Returns a list of {"id": "droplet_001", "num_frames": 87} dicts.
    Returns empty list for single-droplet experiments (no subfolders).
    """
    raw = DATA_ROOT / exp_name / "raw_images"
    if not raw.is_dir():
        return []
    droplets = []
    for d in sorted(raw.iterdir()):
        if not d.is_dir():
            continue
        # Count images inside the subfolder
        n = sum(1 for f in d.iterdir() if f.suffix.lower() in IMAGE_EXTS)
        if n > 0:
            droplets.append({"id": d.name, "num_frames": n})
    return droplets


def resolve_raw_path(exp_name: str, filename: str, droplet: str = None) -> Path:
    """Resolve the full path to an image file, with optional droplet subfolder."""
    base = DATA_ROOT / exp_name / "raw_images"
    if droplet:
        return base / droplet / filename
...
@app.get("/api/droplets/<exp_name>")
def api_list_droplets(exp_name: str):
    """List droplet subfolders inside raw_images/ (gantry multi-droplet experiments).
    Returns [] for single-droplet experiments."""
    return jsonify(list_droplets(exp_name))


@app.get("/api/frames/<exp_name>")
def api_frames(exp_name: str):
    """List frames.  Pass ?droplet=droplet_001 to list frames for a specific droplet."""
    exp_path = DATA_ROOT / exp_name
    if not exp_path.is_dir():
        abort(404)
    droplet = request.args.get("droplet", None)
    frames = scan_experiment(exp_path, droplet=droplet)
    return jsonify(frames)


@app.get("/api/image/<exp_name>/<filename>")
def api_image(exp_name: str, filename: str):
    """Serve an image.  Pass ?droplet=droplet_001 for multi-droplet experiments."""
    droplet = request.args.get("droplet", None)
    img_path = resolve_raw_path(exp_name, filename, droplet)
...
def api_detect_droplet(exp_name: str, filename: str):
    ...
    Pass ?droplet=droplet_001 for multi-droplet experiments."""
    if not HAS_CV2:
...
def api_enhanced_crop(exp_name: str, filename: str):
    ...
    if not ref_filename:
        exp_frames = scan_experiment(DATA_ROOT / exp_name, droplet=droplet)
        ref_filename = exp_frames[0]["filename"] if exp_frames else filename

    cache_key = (exp_name, droplet, ref_filename)
    ref_img = _ref_cache.get(cache_key)
    if ref_img is None:
        ref_path = resolve_raw_path(exp_name, ref_filename, droplet)
...
def api_export_dataset(exp_name: str):
    ...
    ref_img = cv2.imread(str(resolve_raw_path(exp_name, frames[0]["filename"], droplet)))
    ...
    for frame in frames:
        img_path = resolve_raw_path(exp_name, frame["filename"], droplet)
        img = cv2.imread(str(img_path))
        if img is None:
